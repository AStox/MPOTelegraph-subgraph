...
let generateSVG = (
  ...
  let b64 = encode(
    `${start}${middle}${lines}</text><g transform="translate(0,119)"><text class="text fineprint" x="0" y="0">FOR ONE FREE RESPONSE TO THIS TELEGRAM, VISIT THE METAVERAL POST OFFICE AT</text><text class="text fineprint" x="0" y="6">METAVERSALPOSTOFFICE.XYZ WITH THE WALLET HOLDING THIS NON-FUNGIBLE TELEGRAM.</text></g></g></g></g></g><defs><path id="stamp" fill="none" d="M 0,25 A 25 25 0 1 0 0,24.99 z" /><filter id="f2" x="-0.1" y="-0.1" width="200%" height="200%"><feOffset result="offOut" in="SourceAlpha" dx="0" dy="0" /><feGaussianBlur result="blurOut" in="offOut" stdDeviation="10" /><feBlend in="SourceGraphic" in2="blurOut" mode="normal" /></filter></defs></svg>`
  );
...
function parseMessage(message: string): string {
  ...
  const maxLines = 4;
  let numLines = 0;
  log.info("maxLines: {}", [maxLines.toString()]);
  let lines = "";

  let linebreaks = message.split("\n");

  log.info("linebreaks: {}", [linebreaks.length.toString()]);
  log.info("------------------", []);
  for (let i = 0; i < linebreaks.length; i++) {
    if (numLines >= maxLines) break;
    if (linebreaks[i].length <= charPerLine) {
      log.info("linebreak {} fits! Adding. length: {}, content: {}", [
        i.toString(),
        linebreaks[i].length.toString(),
        linebreaks[i],
      ]);
      let lineMsg = linebreaks[i].slice(0, i32(charPerLine));
      log.info("lineMsg: {}", [lineMsg]);
      let line = `<tspan x="0" dy="20"><tspan class="muted">></tspan><tspan x="15">${escapeHtml(
        lineMsg
      )}</tspan></tspan>`;
      numLines++;
      lines = `${lines}${line}`;
    } else {
      log.info("linebreak {} too long. length: {}, content: {}", [
        i.toString(),
        linebreaks[i].length.toString(),
        linebreaks[i],
      ]);
      for (let j = 0; j < linebreaks[i].length / charPerLine; j++) {
        if (numLines >= maxLines) break;
        let lineMsg = linebreaks[i].slice(j * i32(charPerLine), (j + 1) * i32(charPerLine));
        log.info("lineMsg: {}", [lineMsg]);
        let line = `<tspan x="0" dy="20"><tspan class="muted">></tspan><tspan x="15">${escapeHtml(
          lineMsg
        )}</tspan></tspan>`;
...
function encode(input: string): string {
  let output = "";
  let chr1: number, chr2: number, chr3: number, enc1: i32, enc2: i32, enc3: i32, enc4: i32;
  let i = 0;
  ...
  while (i < input.length) {
    ...
    //     letter3.toString(),
    //   ]);
    //   log.info("chr1: {}, chr2: {}, chr3: {}", [chr1.toString(), chr2.toString(), chr3.toString()]);
